...
set -x
TURBULENCE_BOSH_JOBS=$(vault read --format=json secret/turbulence-bosh-jobs $VAULT_HASH_PROPS | jq .turbulence-bosh-jobs)
TURBULENCE_API_PASSWORD=$(vault read --format=json secret/turbulence-ca $VAULT_HASH_PROPS | jq .turbulence-ca)
TURBULENCE_API_IP=$(vault read --format=json secret/turbulence-api-ip $VAULT_HASH_PROPS | jq .turbulence-api-ip )
